...
from preprocess import readProtTerm, read_embeddings
...
def create_dataset(prot_ids, annot_ids, prot_term_edge_index, prot_feature=None, annot_features=None):
    data = HeteroData()
    ...
    if annot_features is not None:
        data["term"].x = annot_features
...
unique_protein_id, unique_annot_id, edge_index_protein_to_term = get_mappings(train_terms_df, train_prot_ids, train_terms_df.term.unique())
data = create_dataset(unique_protein_id, unique_annot_id, edge_index_protein_to_term, prot_feature=protein_features)
...
print(history)
torch.save(model.state_dict(), "light-heteroGNN.torch")
